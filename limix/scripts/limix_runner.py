...
class LIMIX_runner(object):
    ...
    def parse_args(self):
        ...
        parser.add_option(
            ...
            dest='data_script',
            type=str,
            help='A python script for loading the data',
            default=r"example_scripts/example_data.py")
        parser.add_option(
            "-E",
            "--experiment_script",
            action="store",
            ...
            type=str,
            help='A python script for running the experiment',
            default=r"example_scripts/gwas_example.py")
        parser.add_option(
            "-O",
            "--outpath",
            action="store",
        ...
        parser.add_option(
            ...
            action="store_true",
            dest='timestamp',
            help="Append unique timestamp to output value",
            default=False)
        parser.add_option(
            "-R",
            "--delimiter",
            ...
            dest='delimiter',
            type=str,
            help="The delimiter between output values",
            default="\t")
        parser.add_option(
            "-F",
            "--float_format",
            ...
            dest='float_format',
            type=str,
            help="Formating string for floating point output values",
            default="%.6e")
    ...
    def write_resultfiles(self):
        ...
        if len(self.options.outpath) >= 3 and self.options.outpath[
                -3:] == ".h5":
            writer.write_hdf5(
            ...
            writer.write_txt(
                outdir=self.options.outpath,
                timestamp=self.options.timestamp,
                delimiter=self.options.delimiter,
                float_format=self.options.float_format)
        t1 = time.time()
        print(("Elapsed time for writing the output files is %.2f seconds" %
               (t1 - t0)))


def entry_point():
    infostring = "limix_runner.py, Copyright(c) 2014, The LIMIX developers\nlast modified: %s" % time.ctime(
