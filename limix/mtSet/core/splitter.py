    ...
    def splitGeno(
        ...
        if read_from_file:
            ...
            if method == 'slidingWindow':
                self._splitGenoSlidingWindow(
                    size=size, step=step, minSnps=minSnps, maxSnps=maxSnps)
            elif method == 'geneWindow':
                self._splitGenoGeneWindow(
