...
def standardize(snps):
    ...
    # standardize column-wise to mean 0 and std 1
    N = (mdat - SP.mean(mdat, axis=0)) / SP.std(mdat, axis=0)
...
def readBED(
    ...
    if bim is None:
        bim = readBIM(basefilename, usecols=(0, 1, 2, 3))
    if fam is None:
        fam = readFAM(basefilename, usecols=(0, 1))
